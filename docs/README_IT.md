...
| **🚀 Modalità Agente Intelligente** | L'IA pianifica ed esegue autonomamente compiti complessi come la ricerca, la creazione, la modifica, l'eliminazione/spostamento di note e l'esecuzione di codice in una sandbox utilizzando 23 strumenti MCP integrati. |
...
- **🖥️ Modalità Server (Guidata da IA Esterna):**
  - Fornisce 23 strumenti che consentono agli assistenti IA esterni (Claude, Cursor, etc.) o all'IA in Modalità Agente di accedere direttamente al tuo vault.
  ...
  - **Scrittura e Modifica:** `create_note`, `append_to_note`, `replace_note`, `patch_note`, `update_frontmatter`, `save_attachment`, `create_canvas`, `generate_moc` (creazione/modifica di note/canvas, generazione di note MOC (Map of Content) e salvataggio di file binari).
