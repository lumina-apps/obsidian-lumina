...
- **오프라인 보안:** 로컬 임베딩(`ibm-granite`)을 기본 지원합니다. 클라우드 모델을 선택하지 않는 한, 내 소중한 노트 데이터는 외부 서버로 절대 유출되지 않습니다. *(⚠️ 모바일 환경에서는 로컬 임베딩이 지원되지 않습니다.)*
...
- **사용 방법:** 플러그인 설정 메뉴에서 MCP 기능을 켜고, 클라이언트/서버 전송 방식(stdio/SSE)을 구성하세요.
- **주의 사항:** *에이전트 및 MCP 기능은 아직 실험적(Beta) 단계이며, 모바일 환경에서는 Stdio 연결이 지원되지 않습니다. 덮어쓰기 방지 및 글자 수 제한 등 다양한 안전장치가 구현되어 있으나, 외부 AI가 노트를 직접 편집하므로 처음에는 동작을 가볍게 확인하며 사용하시는 것을 권장합니다.*
</details>
