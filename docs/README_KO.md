...
| **🚀 스마트 에이전트 모드** | AI가 스스로 계획을 세우고 23가지 MCP 도구들을 활용해 노트 검색, 생성, 수정, 삭제/이동 및 샌드박스 내 코드 실행 등 복잡한 작업을 자율적으로 수행합니다. |
...
- **오프라인 보안:** Lumina의 지식 검색(RAG) 시스템은 100% 오프라인 로컬 임베딩(내장 `ibm-granite` 다국어 임베딩 모델)을 사용하여 내 노트를 분석합니다. 클라우드 LLM 모델을 사용할 때도 문맥에 필요한 최소한의 데이터만 전송되며, 완벽한 오프라인 환경을 원할 경우 언제든 로컬 LLM으로 전환할 수 있습니다.
- **완전 자동화:** 별도의 설정이 필요 없습니다! 플러그인을 활성화하는 즉시 백그라운드 인덱싱이 조용히 시작되며, 노트를 수정할 때마다 실시간(`watch` 모드)으로 알아서 동기화됩니다.
</details>

<details>
<summary><b>🔗 스마트 탐색 (Smart Discovery)</b></summary>

- **기능 설명:** RAG 엔진을 기반으로, 현재 작성 중인 노트와 관련성이 높은 정보들을 사이드바 패널의 '스마트 탐색' 탭에서 한눈에 제공합니다.
- **주요 기능:**
  - **의미 기반 검색:** 단순 키워드 매칭을 넘어 입력한 문장의 문맥과 의미를 분석해 유사 노트를 시맨틱 검색합니다.
  - **중복 문서 감지:** 내용이 매우 유사한 문서가 이미 보관소 내에 존재할 경우 경고를 표시해, 정보 파편화와 중복 작성을 방지합니다.
  - **추천 태그 및 연관 노트:** 작성 중인 노트의 맥락을 분석하여 적절한 태그를 추천하고 연관된 노트를 실시간으로 제안합니다.
  - **원클릭 연동 및 대화:** 추천된 태그나 연관 노트를 클릭 한 번으로 내 본문에 태그/마크다운 링크(`[[노트명]]`)로 삽입할 수 있으며, 선택한 노트들을 컨텍스트 바구니에 담아 즉시 AI 대화를 시작할 수 있습니다.
- **사용 방법:** 왼쪽 리본 메뉴의 💬 아이콘을 클릭해 사이드 패널을 열고, 상단의 🔗(스마트 탐색) 탭으로 전환하여 사용하세요.
</details>
...
- **기능 설명:** 활성화 시 LLM이 23가지 MCP 도구들을 자율적으로 판단하고 조합하여 작업을 수행합니다. 노트 검색, 읽기, RAG 검색부터 작성, 수정, 파일 관리(삭제/이동), 샌드박스 내 코드 실행, 데일리 노트 연동 등 복잡한 다단계 작업을 스스로 완수할 수 있습니다.
...
<details>
<summary><b>🔌 MCP 연동 (클라이언트 & 서버 양방향 지원)</b></summary>
...
- **🖥️ 서버 모드 (외부 AI가 주도):**
  - 외부 AI(Claude, Cursor 등) 또는 에이전트모드의 AI가 내 볼트에 직접 접근할 수 있도록 23가지 도구 제공
  ...
  - **작성 및 수정:** `create_note`, `append_to_note`, `replace_note`, `patch_note`, `update_frontmatter`, `save_attachment`, `create_canvas`, `generate_moc` (노트 및 캔버스 생성/수정, MOC 허브 노트 생성, 바이너리 파일 저장)
