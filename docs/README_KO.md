...
| **스마트 에이전트 모드** | AI가 스스로 계획을 세우고 다양한 MCP 도구들을 활용해 노트 검색, 생성, 수정, 삭제/이동 및 샌드박스 내 코드 실행 등 복잡한 작업을 자율적으로 수행합니다. |
...
1. **설정 > Connections(연결)**에서 **터미널 CLI 에이전트**(`Claude Code`, `Antigravity`, `OpenCode`, `Codex`), 로컬 LLM 또는 클라우드 AI를 연결합니다.
2. 사이드 패널 채팅에서 CLI 에이전트를 선택(**👁️ 읽기 전용** / **✏️ 편집 모드** 지원)하거나, 대화창에 `/mcp`를 입력하여 **스마트 에이전트 모드**를 활성화합니다.
...
- **사용 방법:** 왼쪽 리본 메뉴의 채팅 아이콘을 클릭하거나, 명령어 팔레트에서 `Lumina: Chat 열기`를 실행하세요. 
...
- **사용 방법:** 왼쪽 리본 메뉴의 채팅 아이콘을 클릭해 사이드 패널을 열고, 상단의 스마트 탐색 탭으로 전환하여 사용하세요.
</details>
