...
| **🚀 智能代理模式** | AI 自主規劃並執行複雜的任務，如使用 23 種內置 MCP 工具進行筆記搜尋、建立、修改、刪除/移動以及在沙盒內執行程式碼。 |
...
- **描述:** 啟用後，LLM 會自主判斷並協調 23 種內置 MCP 工具來执行任務。它可以透過結合筆記搜尋、讀取、寫入、RAG 檢索、沙盒程式碼執行以及每日筆記整合來完成複雜的多步操作。
...
- **🖥️ 伺服器端模式 (外部 AI 主導):**
  - 提供 23 個工具，允許外部 AI 助手（Claude、Cursor 等）或代理模式的 AI 直接存取您的儲存庫。
  ...
  - **編寫與修改:** `create_note`, `append_to_note`, `replace_note`, `patch_note`, `update_frontmatter`, `save_attachment`, `create_canvas`, `generate_moc` (建立/修改筆記和畫布檔案、產生 MOC 核心筆記以及儲存二進位檔案)。
