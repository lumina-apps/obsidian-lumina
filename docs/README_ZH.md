...
| **🚀 智能代理模式** | AI 自主规划并执行复杂的任务，如使用 21 种内置 MCP 工具进行笔记搜索、创建、修改、删除/移动以及在沙盒内执行代码。 |
...
- **描述:** 启用后，LLM 会自主判断并协调 21 种内置 MCP 工具来执行任务。它可以通过结合笔记搜索、读取、写入、RAG 检索、沙盒代码执行以及每日笔记集成来完成复杂的多步操作。
