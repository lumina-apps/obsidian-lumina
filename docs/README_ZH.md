...
| **🚀 智能代理模式** | AI 自主规划并执行复杂的任务，如使用 25 种内置 MCP 工具进行笔记搜索、创建、修改、删除/移动以及在沙盒内执行代码。 |
...
- **描述:** 启用后，LLM 会自主判断并协调 25 种内置 MCP 工具来执行任务。它可以通过结合笔记搜索、读取、写入、RAG 检索、沙盒代码执行以及每日笔记集成来完成复杂的多步操作。
...
- **🖥️ 服务端模式 (外部 AI 主导):**
  - 提供 25 个工具，允许外部 AI 助手（Claude、Cursor 等）或代理模式的 AI 直接访问您的库。
  - **网页搜索:** `lumina_web_search` (使用Tavily、Exa、Google等各种搜索引擎进行实时互联网信息搜索，支持本地LLM的智能截断)
