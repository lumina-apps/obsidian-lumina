...
| **🚀 Modo Agente Inteligente** | Os LLMs planejam e executam de forma autônoma tarefas complexas, como busca, criação, modificação, exclusão/movimentação de notas e execução de código em um sandbox usando 23 ferramentas MCP integradas. |
...
- **Descrição:** Quando ativado, o LLM determina e orquestra de forma autônoma 23 ferramentas MCP integradas para realizar tarefas. Ele pode concluir operações complexas de várias etapas combinando busca, leitura e escrita de notas, recuperação RAG, execução de código em sandbox e integração de notas diárias.
...
- **🖥️ Modo Servidor (Liderado por IA Externa):**
  - Fornece 23 ferramentas que permitem que assistentes de IA externos (Claude, Cursor, etc.) ou a IA no Modo Agente acedam diretamente ao seu cofre.
  ...
  - **Escrita e Modificação:** `create_note`, `append_to_note`, `replace_note`, `patch_note`, `update_frontmatter`, `save_attachment`, `create_canvas`, `generate_moc` (criar/modificar notas/telas (canvas), gerar notas MOC (Map of Content) e guardar ficheiros binários).
