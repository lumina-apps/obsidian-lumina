...
1. **設定 > 接続（Connections）**で**ターミナルCLIエージェント**（`Claude Code`、`Antigravity`、`OpenCode`、`Codex`）、ローカルLLM、またはクラウドAIを接続します。
2. サイドパネルのチャットでCLIエージェントを選択（**👁️ 読み取り専用** / **✏️ 編集モード**対応）するか、`/mcp` と入力して**スマートエージェントモード**を起動します。
