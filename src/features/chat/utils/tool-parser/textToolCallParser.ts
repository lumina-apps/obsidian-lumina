...
function buildClosedTagRegex(): RegExp {
	return new RegExp(
...
function buildOpenTagRegex(): RegExp {
	return new RegExp(`[<$]*(${TOOL_CALL_TAG_ALT})[>]*\\s+([\\s\\S]{1,})$`, 'i');
}
...
export function parseTextToolCalls(content: string): {
	...
	const closedRegex = buildClosedTagRegex();
	const parts: string[] = [];
	let lastEnd = 0;
	...
	if (toolCalls.length === 0) {
		const openTagRegex = buildOpenTagRegex();
